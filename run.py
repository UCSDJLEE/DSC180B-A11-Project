...
from sv_mass_distribution import sv_masss_distribution
...
def main(targets):
    ...
    if 'test' in targets:
        ...
        msg2 = f'In {len(qcd_eda_sets)} QCD files, there are {df_qcd.shape[0]} jet observations while in {len(signal_eda_sets)} Signal files, there are {df_signal.shape[0]} jets'
        print(msg2, '\n')
